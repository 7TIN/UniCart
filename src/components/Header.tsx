...
const Header : React.FC<HeaderProps> = ({
    ...
    const hasActiveFilters = () => {
        return !!filters.category || !!filters.source || !!filters.priceRange || !!searchQuery;
    };
    ...
    return (
        <header className="sticky top-0 z-10 bg-white border-b border-gray-200 px-4 py-4">
            <div className="flex flex-col gap-4">
                ...
                <button type="button" onClick={() => setShowFilters(!showFilters)} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600">
                    <Filter size={18} />
                </button>
                ...
                {showFilters && (
                    <div className="bg-gray-50 rounded-lg p-4 mb-4 text-sm"> 
                        <div className="flex justify-between items-center mb-3"> 
                            ...
                            {hasActiveFilters() && (
                                <button onClick={clearFilters} className="text-xs text-blue-500 hover:text-blue-600 flex items-center">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            <div>
                                <label className="block text-gray-600 mb-1">Website</label>
                                    <select value={filters.source || ''} onChange={(e) => updateFilters({ source: e.target.value || undefined })}
                                    className="w-full p-2 border border-gray-200 rounded-md text-sm">
                ...
                <div className="flex border-b border-gray-200">
                    <button className={`flex items-center py-2 px-4 border-b-2 font-medium text-sm 
                ${
                activeTab === 'cart'
