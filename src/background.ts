...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    ...
    if (message.type === 'ADD_PRODUCT'){
        addProduct(message.product).then(response => sendResponse(response))
    ...
    if (message.type === 'REMOVE_PRODUCT') {
        removeProduct(message.productId).then(response => sendResponse(response))
          .catch(error => console.error('Error removing product:', error));
        return true;
    }

    if (message.type === 'UPDATE_PRODUCT') {
        updateProduct(message.product).then(response => sendResponse(response))
        .catch(error => console.error('Error updating product:', error));
        return true;
    }
});

async function addProduct(product: Product) : Promise<{success: boolean; product: Product}> {
...
async function updateProduct(updatedProduct: Product): Promise<{ success: boolean; product?: Product; error?: string }> {
    const {products} = await getProducts();
